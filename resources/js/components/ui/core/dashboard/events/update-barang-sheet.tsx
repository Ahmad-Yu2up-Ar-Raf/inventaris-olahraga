...
interface UpdateTaskSheetProps
  ...
  task: BarangsSchema | null;
}

export function UpdateBarangSheet({ task, ...props }: UpdateTaskSheetProps) {
  ...
  function onSubmit(input: BarangsSchema) {
    ...
    startTransition(async () => {
      try {
        ...
        router.post(route('dashboard.barang.update', task.id), formData, {
          preserveScroll: true,
          preserveState: true,
          forceFormData: true,
          onBefore: (visit) => {
          ...
          onSuccess: (page) => {
            ...
            toast.success('Barang updated successfully', { id: 'update-toast' });
            form.reset();
    ...
    return (
      <Sheet {...props} modal={true}>
        <SheetContent className="flex flex-col gap-6 overflow-y-scroll">
          ...
          <TaskForm<BarangsSchema> form={form} onSubmit={onSubmit} isPending={loading}   >
