...
import { TaskForm } from "./barang-form";
import { BarangsSchema, barangSchema } from "@/lib/validations/validations";
...
export function CreatebarangSheet({ ...props}: type) {
  ...
  const form = useForm<BarangsSchema>({
    mode: "onSubmit", 
    defaultValues: {
      nama: "", 
      },
...
function onSubmit(input: BarangsSchema) {
   
    
    
console.log(input)
    toast.loading("barang creating....", {
      id: "create-barang"
    });
  ...
  startTransition(() => {
    ...
    router.post(route(`dashboard.barang.store`), input, { 
      preserveScroll: true,
      preserveState: true,
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen} modal={true} >
      ...
      <SheetContent className="flex flex-col gap-6 overflow-y-scroll ">
        <SheetHeader className="text-left sm:px-6 space-y-1 bg-background z-30  sticky top-0   p-4 border-b  ">
          <SheetTitle className=" text-lg">Add New <Button type="button"   variant={"outline"} className=" ml-2  px-2.5 text-base capitalize">barang</Button> </SheetTitle>
          <SheetDescription className=" sr-only">
            Fill in the details below to create a new task
          </SheetDescription>
        </SheetHeader>
        <TaskForm type="barang" isPending={loading} form={form}   onSubmit={onSubmit}>
          <SheetFooter className="gap-3 px-3 py-4 w-full flex-row justify-end  flex  border-t sm:space-x-0">
            ...
            <Button disabled={loading} type="submit" className="w-fit dark:bg-primary !pointer-barang-auto  dark:text-primary-foreground  bg-primary text-primary-foreground " size={"sm"}>
              {loading && <Loader className="animate-spin" />}
              Add
            </Button>
          </SheetFooter>
...
return(
   ...
   <DrawerTrigger asChild className={cn(props.trigger == false && "sr-only" )} >
      ...
      <DrawerContent className="flex flex-col  ">
        ...
        <DrawerTitle className=" text-xl">Add New <Button type="button"   variant={"outline"} className=" ml-2  px-2.5 text-base">barang</Button> </DrawerTitle>
